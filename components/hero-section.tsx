...
export function HeroSection() {
  ...
  useEffect(() => {
    ...
    // We call it right away; if blocked (e.g. Low Power Mode), we retry on first touch.
    const attemptPlay = () => {
      vid.play().catch(() => {
        const retry = () => {
          vid.play().catch(() => {})
  ...
  return (
    ...
    >
      <style>{`
        ...
        /* ── Video always visible — no opacity gate that could hide it on iOS ── */
        .hero-video {
          pointer-events: none;
        }
        /* Strip every native iOS control / overlay */
        ...
        .btn-primary:hover { transform:translateY(-2px); box-shadow:0 12px 40px rgba(201,168,76,.45); background:linear-gradient(135deg,#e8c97a 0%,#c9a84c 50%,#e8c97a 100%); }
        .btn-secondary {
        ...
        .scroll-line {
          width:1px; height:48px; background:linear-gradient(to bottom,rgba(255,255,255,.6),transparent);
