...
const PRIMARY_TEL  = '+91-9744201662'    // display / tel: link — primary
const ALT_TEL      = '+91-9895023973'    // secondary / alternative call number

const activities = [
  { value: 'country-boating', label: '⛵ Country Boating — ₹1,700' },
  { value: 'kayaking', label: '🛶 Kayaking — ₹700' },
  { value: 'stand-up-paddle', label: '🏄 Stand Up Paddle — ₹1,000' },
  { value: 'coracle-ride', label: '🪵 Coracle Ride — ₹300' },
  { value: 'atv-ride', label: '🏍️ ATV Ride — ₹300' },
  { value: 'speed-boat', label: '🚤 Speed Boat — ₹3,500' },
]
...
export function ContactSection() {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    const waURL = `https://wa.me/${PRIMARY_WA}?text=${encodeURIComponent(waMessage)}`
    window.open(waURL, '_blank')
            ...
            <div>
              ...
              <div className="info-item">
                ...
                <div>
                  <div className="info-title">Phone</div>
                  {/* PRIMARY number */}
                  <a href={`tel:${PRIMARY_TEL}`} className="info-value">+91 97442 01662</a>
                  <div style={{ fontFamily: "'Jost', sans-serif", fontSize: '0.72rem', color: '#999', marginTop: 3 }}>
                    Alt: <a href={`tel:${ALT_TEL}`} style={{ color: '#999', textDecoration: 'none' }}>+91 98950 23973</a>
                  </div>
                </div>
              </div>
