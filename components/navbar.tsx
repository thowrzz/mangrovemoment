...
import { useState, useEffect } from 'react'
import Link from 'next/link'
import { Menu, MessageCircle, ChevronDown, X, Sailboat } from 'lucide-react'
import {
  DropdownMenu,
  DropdownMenuContent,
  DropdownMenuItem,
...
import { MdSpeed, MdRowing } from 'react-icons/md'
import { TbMotorbike, TbStretching } from 'react-icons/tb'
  ...
  return (
    <>
      <style>{`
        ...
        .mobile-activity-link:hover { color: #e8d49a; }
        ...
        .activity-icon { color: #c9a84c; flex-shrink: 0; }
      `}</style>
      ...
      <nav className={`nav-root fixed top-0 left-0 right-0 z-50 transition-all duration-500 ${isScrolled ? 'nav-scrolled' : 'nav-transparent'}`}>
        <div className="max-w-7xl mx-auto px-5 sm:px-8 lg:px-10">
          <div className="flex justify-between items-center h-[72px]">
            ...
            {/* ── Right CTAs — PRIMARY number ── */}
            <div className="hidden md:flex items-center gap-4">
              <a href={`https://wa.me/${PRIMARY_WA}`} target="_blank" rel="noopener noreferrer" className="btn-wa" aria-label="WhatsApp">
                <MessageCircle size={17} />
              </a>
              <div style={{ width: '1px', height: '20px', background: 'rgba(255,255,255,0.15)' }} />
              <a
                href={`https://wa.me/${PRIMARY_WA}?text=Hi!%20I%20would%20like%20to%20book%20an%20experience%20at%20Paravur%20Backwaters.`}
                target="_blank"
                rel="noopener noreferrer"
                className="btn-book"
              >
              ...
              <SheetTrigger asChild>
                ...
                >
                  <span style={{
                    ...
                    <span style={{ width: 18, height: 1.5, background: '#fff', borderRadius: 2, display: 'block' }} />
                    <span style={{ width: 13, height: 1.5, background: '#fff', borderRadius: 2, display: 'block' }} />
                    <span style={{ width: 18, height: 1.5, background: '#fff', borderRadius: 2, display: 'block' }} />
                  </span>
                  <span style={{
                    position: 'absolute',
